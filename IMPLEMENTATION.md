...
- **탭 / 칩**: `is_active` + `aria-selected` + 패널 `hidden` 동기화. 선수단 탭은 투수·타자 패널이 각각 독립 슬라이더(`data-scroller="player_pitcher" / "player_hitter"`)라 진행 바가 따로 동작하며, 탭 전환 시 `resize`를 발생시켜 숨어 있던 패널의 진행 바를 다시 계산합니다.
- **스크롤 등장**: `IntersectionObserver` + `.reveal.is_shown`.
- **갤러리 사진 무한 흐름**: 1번 열은 위로, 2번 열은 아래로 초당 32px 이동. 원본 6장을 한 벌 복제해 `(이미지 높이 + 간격) × 6`(Desktop 3150px / Mobile 1782px)마다 순환시켜 이음새가 보이지 않습니다. 마우스를 올리면 속도가 0을 향해 지수적으로 감속(약 1.5초에 정지), 벗어나면 같은 곡선으로 복귀합니다. `animation-play-state`는 즉시 멈춰버려 `requestAnimationFrame`으로 구현했고, 화면 밖이거나 탭이 비활성이면 프레임을 멈춥니다.
...
5. **타자 선수 한글 이름 없음** — 타자 탭은 `img/player/hitter/`의 12장을 모두 사용해 채웠습니다. 다만 Figma 파일 전체(Home 3개 프레임)에 타자 명단이 없어, **파일명에서 확정되는 정보만** 사용했습니다.
   - 등번호: 파일명 앞자리 (`02_kim` → 2)
   - 이름 자리: 파일명의 로마자 성 (Kim, Park, Son, Lee, Ryu, Kang, Yang, Yun, Jung, Park, Park, An)
   - 포지션: `타자` (탭 라벨 기준)
   - 로마자 전체 이름 줄은 데이터가 없어 생략했고, `.profile_name_group { min-height: 37px }`으로 투수 카드와 높이를 동일하게 맞췄습니다.

   실제 선수 이름을 알려주시면 `index.html`의 `#panel_hitter` 안 `<strong>` 12곳만 교체하면 됩니다.
